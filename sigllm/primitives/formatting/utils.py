...
def run_pipeline(method, data=None, 
                         interval=3600, 
                         window_size=15, 
                         verbose=True, 
                         samples=7, 
    ...
    Args:
        ...
        normalize (bool): Whether to normalize the data before running the pipeline.
        multivariate_allowed_symbols (list): The allowed symbols for LLMs to output aside from digits
        pipeline_name: The name of the pipeline we are wrapping (choice of `mistral_detector` and `gpt_detector`)
        stride: The gap between consecutive prediction windows.
        n_clusters: Not yet supported. Will be used with the `binning` pre-processing strategy in the future.
        strategy: For now, must be `scaling`. We will add option for `binning` in the future.
    ...
    test_hyperparameters = {
        ...
        "sigllm.primitives.forecasting.huggingface.HF#1": {
            "samples": samples,
            "temp": temp,
            "multivariate_allowed_symbols": [] if multivariate_allowed_symbols is None else multivariate_allowed_symbols,
            "steps": hf_steps,
        },
    }
    ...
    if strategy == 'scaling':
        ...
        raise ValueError(f"Note that binning is not supported for now.")
    else:
        raise ValueError(f"Invalid strategy: {strategy}. Note that only scaling is supported for now.")
...
def _process_multi_step_results(method, context, pipeline, steps_ahead, verbose):
    ...
    """
    y_hat_by_step = method.format_as_integer(
