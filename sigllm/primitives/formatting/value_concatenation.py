...
class ValueConcatenation(MultivariateFormattingMethod):
    ...
    def format_as_integer(
        self, X: list[str], separator=',', trunc=None, num_dims=None, target_column=None, **kwargs
    ) -> np.ndarray:
        """Extract values for the target dimension from each sample in each window as ints.

        Args:
            X (list[str]):
                list of strings, each string is a concatenation of num_dims values
        ...
        if num_dims is None:
            raise ValueError(
                'Cannot parse concatenated values '
        ...
        result = [
            [
                np.array([int(x) for x in entry.lstrip(separator).split(separator) if x])[
                    target_column::num_dims
                ][:trunc]
                for entry in row
            ]
            for row in X
